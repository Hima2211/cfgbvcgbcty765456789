...
export async function registerRoutes(app: Express, upload?: any): Promise<Server> {
  ...
  app.post('/api/challenges', PrivyAuthMiddleware, async (req: AuthenticatedRequest, res) => {
    try {
      ...
      // Use admin-specific creation to avoid requiring a challenger balance
      const challenge = await storage.createAdminChallenge(challengeData as any);
  ...
  app.post('/api/admin/challenges/:id/result', adminAuth, async (req: AdminAuthRequest, res) => {
    try {
      ...
      // For draws, we don't queue payouts - they're handled in adminSetChallengeResult
      if (result === 'draw') {
        return res.json({ 
          challenge,
