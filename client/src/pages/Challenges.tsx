      ...
      <div className="max-w-7xl mx-auto px-3 md:px-4 sm:px-6 lg:px-8 py-3 md:py-8">
        ...
        <Tabs value={selectedTab} onValueChange={setSelectedTab} className="space-y-6">
          <TabsList className={`grid w-full ${
            user 
              ? (pendingChallenges.length > 0 && awaitingResolutionChallenges.length > 0 
                  ? 'grid-cols-5'
                  : pendingChallenges.length > 0 || awaitingResolutionChallenges.length > 0
                  ? 'grid-cols-4'
                  : 'grid-cols-3')
            ...
            {user && pendingChallenges.length > 0 && (
              <TabsTrigger value="pending">
                Pending
              </TabsTrigger>
            )}
            {user && awaitingResolutionChallenges.length > 0 && (
              <TabsTrigger value="awaiting_resolution">
                Awaiting
              </TabsTrigger>
            )}
            ...
            {isLoading ? (
              ...
              <Card className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700">
                <CardContent className="text-center py-12">
                  <i
                    className="fas fa-star text-4xl mb-4"
                    style={{ color: "#7440ff" }}
                  ></i>
                  ...
                  <p className="text-slate-600 dark:text-slate-400">
                    Check back later for exciting featured challenges!
                  </p>
                </CardContent>
              </Card>
            ) : (
          ...
          <TabsContent value="awaiting_resolution" className="space-y-2 md:space-y-2">
            {awaitingResolutionChallenges.length === 0 ? (
              <Card className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700">
                <CardContent className="text-center py-12">
                  <i
                    className="fas fa-hourglass-end text-4xl mb-4"
                    style={{ color: "#7440ff" }}
                  ></i>
                  <h3 className="text-xl font-semibold text-slate-900 dark:text-slate-100 mb-2">
                    No challenges awaiting resolution
                  </h3>
                  <p className="text-slate-600 dark:text-slate-400">
                    Challenges will appear here when they end and need your decision.
                  </p>
                </CardContent>
              </Card>
            ) : (
